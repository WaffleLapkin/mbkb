...
#[derive(Debug, Copy, Clone, PartialEq, Eq, PartialOrd, Ord, enumn::N)]
#[repr(u8)]
pub enum KeyCode {
...
impl KeyCode {
    /// Testing utility: given an ASCII character code returns a key code you
    /// need to press to type this character (assuming QWERTY layout) alongside
    /// with whatever you need to press shift ot not.
    pub fn from_ascii(char: u8) -> Option<(Self, bool)> {
        let code = match char {
            b'a'..=b'z' => return Self::n(char + (Self::A as u8) - b'a').map(|k| (k, false)),
            b'A'..=b'Z' => return Self::n(char + (Self::A as u8) - b'A').map(|k| (k, true)),
            b' ' => Self::Space,
            b',' => Self::Comma,
            b'.' => Self::Dot,
            b'\n' => Self::Enter,
